...
TxProposal.prototype._addSignaturesToBitcoreTx = function(tx, signatures, xpub) {
  ...
  var botcore = Bitcore[self.coin];

  if (signatures.length != this.inputs.length)
    throw new Error('Number of signatures does not match number of inputs');

  var i = 0,
    x = new botcore.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = self.inputs[i];
    try {
      var signature = botcore.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(self.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: botcore.crypto.Signature.SIGHASH_ALL | botcore.crypto.Signature.SIGHASH_FORKID,
        publicKey: pub,
      };
