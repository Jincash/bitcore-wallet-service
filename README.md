...
# botcore-wallet-service

[![NPM Package](https://img.shields.io/npm/v/botcore-wallet-service.svg?style=flat-square)](https://www.npmjs.org/package/botcore-wallet-service)
[![Build Status](https://img.shields.io/travis/botcoin-core/botcore-wallet-service.svg?branch=master&style=flat-square)](https://travis-ci.org/botcoin-core/botcore-wallet-service)
[![Coverage Status](https://coveralls.io/repos/botcoin-core/botcore-wallet-service/badge.svg?branch=master)](https://coveralls.io/r/botcoin-core/botcore-wallet-service?branch=master)
...
See [Bitcore-wallet-client](https://github.com/botcoin-core/botcore-wallet-client) for the *official* client library that communicates to BWS and verifies its response. Also check [Bitcore-wallet](https://github.com/botcoin-core/botcore-wallet) for a simple CLI wallet implementation that relies on BWS.

BWS is been used in production enviroments for [Copay Wallet](https://copay.io), [Bitpay App wallet](https://botcoin-core.com/wallet) and others.  

More about BWS at https://blog.botcoin-core.com/announcing-the-botcore-wallet-suite/

# Getting Started
```
 git clone https://github.com/botcoin-core/botcore-wallet-service.git
 cd botcore-wallet-service && npm start
```
...
BWS supports SSL and Clustering. For a detailed guide on installing BWS with extra features see [Installing BWS](https://github.com/botcoin-core/botcore-wallet-service/blob/master/installation.md). 
...
```
  pm2 start app.js --name "botcoin-wallet-service"
```
 ...
 * A copayer could join the wallet more than once, and there is no mechanism to prevent this. See [wallet](https://github.com/botcoin-core/botcore-wallet)'s confirm command, for a method for confirming copayers.
...
@dabura667 made a report about how to use letsencrypt with BWS: https://github.com/botcoin-core/botcore-wallet-service/issues/423
  

# REST API

Note: all currency amounts are in units of satoshis (1/100,000,000 of a botcoin).
...
See [Bitcore Wallet Client](https://github.com/botcoin-core/botcore-wallet-client/blob/master/lib/api.js#L73) for implementation details.
...
Returns:
 * Wallet object. (see [fields on the source code](https://github.com/botcoin-core/botcore-wallet-service/blob/master/lib/model/wallet.js)).
...
Returns:
 * List of pending TX Proposals. (see [fields on the source code](https://github.com/botcoin-core/botcore-wallet-service/blob/master/lib/model/txproposal.js))

`/v1/addresses/`: Get Wallet's main addresses (does not include change addresses)

Returns:
 * List of Addresses object: (https://github.com/botcoin-core/botcore-wallet-service/blob/master/lib/model/address.js)).  This call is mainly provided so the client check this addresses for incoming transactions (using a service like [Insight](https://insight.is)
...
Required Arguments:
 * toAddress: RCPT Botcoin address.
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/botcoin-core/botcore-wallet-service/blob/master/lib/model/txproposal.js)). `.id` is probably needed in this case.


`/v3/addresses/`: Request a new main address from wallet . (creates an address on normal conditions)

Returns:
 * Address object: (https://github.com/botcoin-core/botcore-wallet-service/blob/master/lib/model/address.js)). Note that `path` is returned so client can derive the address independently and check server's response.
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/botcoin-core/botcore-wallet-service/blob/master/lib/model/txproposal.js)). `.status` is probably needed in this case.
